...
// of the WHOLE output row. A PCU sees NLANE = 16 of them. This is a dataflow
...
// ---------------------------------------------------------------------------
//
// The binary16 pack of axis 3 is kept by default even though the INT4 path does
// not need it, for two reasons. It is a real requirement -- not every consumer
// of a projection output is an INT4 layer, and lm_head and the residual path
// want the float -- and it makes this directory a strict superset of
// rtl/5_spinquant_dequant_rne, so the two rows differ by the requantizer and
// nothing else. Set KEEP_FP16_OUT = 0 for the minimal INT4-only build; the row
// in synth/run_all.sh uses the superset.
//
// Do not read the requantizer's cost as the subtraction of the two rows'
// areas. Measured, this build uses 1062 more cells and 139 more flip-flops
// than spinquant_pcu_dq and still comes out 274 um2 SMALLER, because ABC maps
// the larger netlist with cheaper cells on average (1.221 against 1.270
// um2/cell) while both meet timing with the same slack. The cell and flop
// deltas are the honest statement; the area totals are not decomposable at
// this scale.
