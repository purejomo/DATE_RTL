...
// Shared binary32 accumulator adder for the AWQ dequantization path.
//
// Byte-for-byte the arithmetic of
// rtl/3_p3llm_dequant_rne/p3llm_dequant_fp32_add_pipe.sv; only the module name
// differs, so the two source sets can be compiled independently. Keeping it a
// copy rather than a shared file is the same rule the raw PCU sources already
// follow: one directory, one unambiguous source set.
//
