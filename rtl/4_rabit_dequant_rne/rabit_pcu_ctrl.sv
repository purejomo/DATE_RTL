...
// PCU sequencer: folded word consumption, accumulator slot select, drain FSM.
//
// One column command delivers one weight word and the PCU spends
// NPATH*NFOLD internal cycles on it.  pump {path, fold} reduces one path and
// one physical-PE-width slice of the logical output word.  NFOLD=1 is the
// original implementation; NFOLD=2 lets four physical PEs consume the same
// 8-output/256-bit mapping in four 500-MHz cycles.
//
...
// them for exactly NPATH*NFOLD cycles, which is what the datapath needs. rd_ready_o
...
module rabit_pcu_ctrl #(
    ...
    output logic [FW-1:0]    fold_sel_o,
    ...
    output logic [FW-1:0]    acc_fold_o,
    ...
    localparam logic [CW-1:0] LAST_PUMP = CW'(NPUMP - 1);
    ...
    always_comb begin
        ...
        pump_c   = start_c ? {CW{1'b0}} : pump_q;
        path_c   = PW'(pump_c / NFOLD);
        fold_c   = FW'(pump_c % NFOLD);
        group_c  = start_c ? rd_group_i : group_q;
        last_c   = active_c && (pump_c == LAST_PUMP);
...
`ifndef SYNTHESIS
    initial begin
        if (NPATH < 1 || NFOLD < 1)
            $fatal(1, "rabit_pcu_ctrl: NPATH and NFOLD must be positive");
        if (NPUMP != (1 << CW))
            $fatal(1, "rabit_pcu_ctrl: NPATH*NFOLD must be a power of two");
    end
`endif
