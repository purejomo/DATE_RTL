...
// mirrored there.
//
// Axis-2 (acc16) build. The only change against the 32-bit sibling in
// rtl/2_awq_p3llm_16pe_v2 is the accumulator: each PE rounds its 4-lane
// partial sum to nearest, ties to even, down to ACC_W bits before adding, and
// holds ACC_W bits of state. Alignment, decode, multipliers, compressor and
// initiation interval are untouched, so the area delta this row reports is
// attributable to the accumulator alone. See int4float_pe.v for the ACC_RSH
// derivation.
//
...
//
// With sixteen PEs one 256-bit bank access is exactly the 64 four-bit weights
// consumed per transaction, so the bank interface needs no slicing. NUM_PES
// stays a parameter, but every wrapper passes it explicitly so that compiling
// this file alongside its eight-PE twin cannot silently change a build.
//
...
module int4float_pcu #(
    ...
    parameter integer ACC_RSH = 12,  // RNE shift before accumulating
    ...
    output wire         o_valid,
    output wire [NUM_PES*ACC_W-1:0] o_acc,    // one signed ACC_W accumulator per PE
