...
// AWQ-HBM-PIM PCU with sixty-four independent INT4 x binary16 MAC lanes.
module awq_int4fp16_pcu_64_lane (
    input  wire          clk,
    input  wire          rst_n,
    input  wire          i_valid,
    ...
    output wire          o_valid,
    output wire [2047:0] o_acc
);
    wire [63:0] lane_valid;

    genvar lane;
    generate
        for (lane = 0; lane < 64; lane = lane + 1) begin : g_lane
            awq_int4fp16_mac_1_lane u_mac (
                .clk          (clk),
                .rst_n        (rst_n),
                .i_valid      (i_valid),
