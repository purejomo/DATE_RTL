...
// Shared P3-LLM dequantization multiplier.
//
// Numeric operation:
//
//   fp32_o = RNE_binary32(fixed_i * scale_i * 2**binary_exp_i)
//
// fixed_i is a two's-complement signed integer.  scale_i is required to be a
// positive, finite binary16 value; +0 and binary16 subnormals are supported.
// A negative scale (including -0), infinity, or NaN is outside that contract,
// asserts invalid_o, and produces the canonical binary32 quiet NaN.
//
...
// complete binary16 significand, and the exact 43-bit product is rounded only
...
module p3llm_dequant_fixed32_fp16_mul_pipe (
    input  logic                      clk,
    input  logic                      rst_n,
    input  logic                      in_valid_i,
    input  logic signed [31:0]        fixed_i,
    input  logic        [15:0]        scale_i,
    input  logic signed  [6:0]        binary_exp_i,
    ...
    // Pack exact_product * 2**base_exp as binary32.  Although the legal input
    // ranges imply a normal binary32 result for every nonzero transaction
    // (-88 <= unbiased exponent <= 109), the subnormal path is retained so
    // that the range handling and flags remain explicit and self-checkable.
    function automatic logic [34:0] pack_exact_fp32 (
        input logic               sign,
        input logic               invalid,
        input logic        [42:0] exact_product,
        ...
        logic        sticky_bit;
        logic        round_up;
        logic [42:0] shifted_product;
        logic [63:0] subnormal_shifted;
        logic [43:0] subnormal_quotient;
        logic [43:0] subnormal_rounded;
        ...
        begin
            ...
            sticky_bit     = 1'b0;
            round_up       = 1'b0;
            shifted_product = 43'd0;
            subnormal_shifted = 64'd0;
            subnormal_quotient = 44'd0;
            subnormal_rounded  = 44'd0;
            leading_index  = -1;
            unbiased_exp   = 0;
            biased_exp_integer = 0;
            base_exp_integer = $signed({{20{base_exp[11]}}, base_exp});
            shift_amount   = 0;
            subnormal_power = 0;

            for (scan = 0; scan < 43; scan = scan + 1) begin
            ...
            if (invalid) begin
                ...
                if (unbiased_exp >= -126) begin
                    ...
                    // discarded from the exact 43-bit product for RNE.
                    ...
                    if (subnormal_power >= 0) begin
                        ...
                        // fits below bit 23.
                        subnormal_shifted =
                            {21'd0, exact_product} << subnormal_power;
                        subnormal_quotient = subnormal_shifted[43:0];
                    end else begin
                        shift_amount = -subnormal_power;
                        if (shift_amount < 43) begin
                            subnormal_quotient =
                                {1'b0, exact_product} >> shift_amount;
                        end else begin
                            subnormal_quotient = 44'd0;
                        end
                        if ((shift_amount >= 1) && (shift_amount <= 43)) begin
                            guard_bit = exact_product[shift_amount-1];
                        end
                        for (scan = 0; scan < 43; scan = scan + 1) begin
                            if (scan < (shift_amount - 1)) begin
    ...
    always_comb begin
        ...
        invalid_d = scale_i[15] || (scale_i[14:10] == 5'h1f);
        base_exp_integer_d =
            $signed({{25{binary_exp_i[6]}}, binary_exp_i});
