...
// Numeric operation:
//
//   fp16_o = RNE_binary16(fp32_i * scale_i)
//
...
// asserts invalid_o and produces canonical binary16 quiet NaN.  Packing uses
// gradual binary16 underflow and round-to-nearest, ties-to-even.
//
// The exact 24-by-11-bit significand product is retained until the one final
// binary16 rounding.  No binary32 multiply rounding is inserted ahead of the
...
module p3llm_dequant_fp32_fp16_mul_pack_pipe (
    input  logic        clk,
    input  logic        rst_n,
    ...
    function automatic logic [18:0] pack_exact_fp16 (
        input logic               sign,
        input logic               invalid,
        ...
        begin
            ...
            if (invalid) begin
                ...
                if (unbiased_exp >= -14) begin
                    shift_amount = leading_index - 10;
                    if (shift_amount > 0) begin
                        shifted_product = exact_product >> shift_amount;
                        retained_sig = shifted_product[10:0];
                    ...
                    if (rounded_sig[11]) begin
                        retained_sig = rounded_sig[11:1];
                        unbiased_exp = unbiased_exp + 1;
                    end else begin
                        retained_sig = rounded_sig[10:0];
                    end

                    if (unbiased_exp > 15) begin
                        // IEEE RNE overflow destination is signed infinity.
                        result = {sign, 5'h1f, 10'd0};
                        overflow_flag = 1'b1;
                    end else begin
                        biased_exp_integer = unbiased_exp + 15;
                        biased_exp = biased_exp_integer[4:0];
                        result = {sign, biased_exp, retained_sig[9:0]};
                    end
                end else begin
                    // binary16 subnormal fraction =
                    // RNE(exact_product * 2**(base_exp + 24)).
                    subnormal_power = base_exp_integer + 24;
                    guard_bit = 1'b0;
                    sticky_bit = 1'b0;
                    if (subnormal_power >= 0) begin
                        // The subnormal condition guarantees this exact shift
                        // remains below the binary16 hidden-bit position.
                        subnormal_shifted =
                    ...
                    subnormal_rounded = subnormal_quotient
                                      + {{35{1'b0}}, round_up};
                    if (subnormal_rounded[10]) begin
                        // RNE promoted the largest tiny result to min-normal.
                        result = {sign, 5'd1, 10'd0};
                    end else begin
                        result = {sign, 5'd0, subnormal_rounded[9:0]};
    ...
    // Stage 1: normalize, gradual-underflow RNE-pack, and register binary16.
    always_ff @(posedge clk) begin
        if (!rst_n) begin
            ...
            if (s0_valid_q) begin
                fp16_o      <= packed_s0[RESULT_LSB +: 16];
